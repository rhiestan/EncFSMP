...
bool fileExists(const char *fileName) {
  efs_stat buf;
  return fs_layer::lstat(fileName, &buf) == 0;
}

bool isDirectory(const char *fileName) {
  efs_stat buf;
  if (fs_layer::lstat(fileName, &buf) == 0) {
...
// If case-insensitive system, opt for Block32 filename encoding
  ...
  if (!saveConfig(Config_V6, rootDir, config.get(), false, "")) {
    return rootInfo;
  }
...
RootPtr initFS(EncFS_Context *ctx, const std::shared_ptr<EncFS_Opts> &opts,
  ...
  if (readConfig(opts->rootDir, config.get(), opts->useExternalConfigFile, opts->externalConfigFileName) != Config_None) {
    ...
    if (config->plainData) {
      if (! opts->insecure) {
        ostr << _("Configuration use plainData but you did not use --insecure\n");
        return rootInfo;
      }
      static Interface NullInterface("nullCipher", 1, 0, 0);
      fsConfig->cipher = Cipher::New(NullInterface, 0);
...
int remountFS(EncFS_Context *ctx) {
  ...
  RootPtr rootInfo = initFS(ctx, ctx->opts, std::cout);
  if (rootInfo) {
