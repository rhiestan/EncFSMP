...
// TODO
bool lock_memory(const void *addr, size_t len)
{
	return false;
}

bool unlock_memory(const void *addr, size_t len)
{
	return false;
}
...
long time_diff(const fs_layer::timeval_fs &end, const fs_layer::timeval_fs &start)
{
...
{
    int iter = 1000;
    fs_layer::timeval_fs start, end;

    for(;;)
    {
        fs_layer::gettimeofday( &start, 0 );
...
SSLKey::SSLKey(int keySize_, int ivLength_)
{
    ...
    lock_memory( buffer, keySize + ivLength );
}
...
SSLKey::~SSLKey()
{
    ...
    OPENSSL_free( buffer );
    unlock_memory( buffer, keySize + ivLength );
...
Interface SSL_Cipher::get_interface() const
{
    return realIface;
}
	...
	uint64_t iv64, const CipherKey &ckey ) const
{
    ...
    if(blockMod != 0)
	throw RL_ERROR("Invalid data size, not multiple of block size");
